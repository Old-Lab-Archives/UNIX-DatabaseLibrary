...
/* Defining hash chains and free list chain in index file */
#define PTR_SZ 6 /* size of ptr field in hash chain */
#define PTR_MAX 999999 /* Maximum offset(file size) = 10**PTR_SZ-1 */
#define NHASH_DEF 137 /* default hash table size */
#define FREE_OFF 0 /* offset of ptrto free list in index file */
...
typedef struct
{
	...
	char *idxbuf; /* memory allocated (malloc) buffer for index record */
	char *datbuf; /* memory allocated (malloc) buffer for data record */
	char *name; /* name db was opened */
	...
	long cnt_fetchok; /* fetch okay =P */
	long cnt_fetcherror; /* fetch error :( */
	...
	long cnt_stor4; /* store : DB_REPLACE, same data len, overwrite */
